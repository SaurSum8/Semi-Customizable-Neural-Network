...
public class AIO_Network {
	...
	//Dynamic Step
	double stepLimit = -0.00001;
	double prevAccDS = 0.0;
	double changeThresPERC = 1.0;
	...
	public void forward() {
		...
		for(int i = 0; i < hiddenNeurons[0].length; i++) {
			...
			hiddenNeurons[0][i] = operation.dot(inputs, inpTOhid[i]) + biasHid[0][i];
		...
		for(int i = 1; i < hiddenNeurons.length; i++) {
			...
			for(int j = 0; j < hiddenNeurons[0].length; j++) {
				...
				hiddenNeurons[i][j] = operation.dot(hiddenNeurons[i - 1], hidTOhid[i - 1][j]) + biasHid[i][j];
